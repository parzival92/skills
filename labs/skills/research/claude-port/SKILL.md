...
All state lives in `~/.devops-research/`:
...
If `~/.devops-research/profile.md` does not exist, run first-time setup before the
...
- **Persist before ending the turn.** A mode is not finished until its writes (read-log,
  ...
  `~/.devops-research` is a git repo, also commit the writes with message
